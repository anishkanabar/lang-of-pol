...
def calculate_detection_error_rate(test_prediction, protocol):
    """
    ...
    metric.report().to_csv(f'/Users/shiyang/Desktop/NIH/git/asr/results/shiyang_test/vad_{generate_ramdom_code()}.csv')
    return 'detection error rate', abs(metric)


def calculate_diarization_error_rate(test_prediction, protocol):
    """
    ...
    for hypothesis, reference in zip(test_prediction, protocol.test()):
        _ = metric(reference['annotation'], hypothesis, uem=reference['annoated'])
    # save result to a csv file
    metric.report().to_csv(f'/Users/shiyang/Desktop/NIH/git/asr/results/shiyang_test/dia_{generate_ramdom_code()}.csv')
    return 'diarization error rate', abs(metric)

def calculate_segmentation_precision(test_prediction, protocol):
    """
    ...
    for hypothesis, reference in zip(test_prediction, protocol.test()):
        _ = metric(reference['annotation'], hypothesis, uem=reference['annoated'])
    # save result to a csv file
    metric.report().to_csv(f'/Users/shiyang/Desktop/NIH/git/asr/results/shiyang_test/scd_{generate_ramdom_code()}.csv')
    return 'segmentation precision', abs(metric)


def test_performance(pipeline, protocol, metric):
    """
    ...
    name, result = metric(test_prediction, protocol)
